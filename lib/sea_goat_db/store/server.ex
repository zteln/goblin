defmodule SeaGoatDB.Store.Server do
  @moduledoc false
  use GenServer
  ...
  defimpl SeaGoatDB.Store do
    def put(%{server: store}, file, level_key, bloom_filter, priority, size, key_range) do
      GenServer.call(store, {:put, file, level_key, bloom_filter, priority, size, key_range})
    end

    def remove(%{server: store}, file) do
      GenServer.call(store, {:remove, file})
    end

    def get(%{server: store}, key) do
      GenServer.call(store, {:get, key})
    end

    def new_file(%{server: store}) do
