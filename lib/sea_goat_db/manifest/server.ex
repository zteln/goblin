defmodule SeaGoatDB.Manifest.Server do
  @moduledoc false
  use GenServer

  @manifest_name :sea_goat_db_manifest
  ...
  @type version :: %{
          files: MapSet.t(SeaGoatDB.db_file()),
          count: non_neg_integer(),
          seq: SeaGoatDB.db_sequence()
        }
  ...
  defimpl SeaGoatDB.Manifest do
    def log_rotation(%{server: manifest}, wal) do
      GenServer.call(manifest, {:log_edit, {:wal_added, wal}})
    end

    def log_flush(%{server: manifest}, files, wal) do
    ...
    def log_sequence(%{server: manifest}, seq) do
      GenServer.call(manifest, {:log_edit, {:seq, seq}})
    end

    def log_compaction(%{server: manifest}, rotated_files, new_files) do
    ...
    def get_version(%{server: manifest}, keys \\ []) do
