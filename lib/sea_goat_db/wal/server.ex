defmodule SeaGoatDB.WAL.Server do
  @moduledoc false
  use GenServer
  ...
  defimpl SeaGoatDB.WAL do
    def sync(%{server: wal}) do
      GenServer.call(wal, :sync_now)
    end

    def append(%{server: wal}, buffer) do
      GenServer.call(wal, {:append, buffer})
    end

    def rotate(%{server: wal}) do
      GenServer.call(wal, :rotate)
    end

    def clean(%{server: wal}, rotated_file) do
      GenServer.call(wal, {:clean, rotated_file})
    end

    def recover(%{server: wal}) do
