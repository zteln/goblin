defmodule Talon do
  ...
  def put(db, key, value) do
    writer = name(db, :writer)
    Talon.Writer.put(writer, key, value)
  end

  def put_multi(db, pairs) do
    writer = name(db, :writer)
    Talon.Writer.put_multi(writer, pairs)
  end

  @spec remove(db_server(), db_key()) :: :ok
  def remove(db, key) do
    writer = name(db, :writer)
    Talon.Writer.remove(writer, key)
  end

  def remove_multi(db, keys) do
    writer = name(db, :writer)
    Talon.Writer.remove_multi(writer, keys)
  end
  ...
  def get(db, key, default \\ nil) do
    ...
    case Talon.Reader.get(key, writer, store) do
      :not_found -> default
      {_seq, value} -> value
    end
  end
  ...
  def init(args) do
    ...
    children = [
      ...
      {Talon.RWLocks, Keyword.merge(args, name: rw_locks_name)},
      {Talon.Manifest, Keyword.merge(args, name: manifest_name, db_dir: db_dir)},
      {Talon.WAL, Keyword.merge(args, name: wal_name, db_dir: db_dir)},
      {Talon.Compactor,
       Keyword.merge(args,
         name: compactor_name,
         key_limit: key_limit,
  ...
  defp name(name, suffix) do
    if name, do: :"#{name}_#{suffix}", else: :"talon_#{suffix}"
  end
end
