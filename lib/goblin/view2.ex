defmodule Goblin.View2 do
  ...
  def soft_delete_table(ref, key) do
    case :ets.match(ref, {{:table, :"$1", key}, :_, :_, :_}) do
      ...
      [[alive]] ->
        claim = make_ref()
        :ets.insert(ref, {{:pending_delete, claim}})
        dead = inc_get_epoch(ref)
        :ets.update_element(ref, {:table, alive, key}, {2, dead})
        :ets.delete(ref, {:pending_delete, claim})
    end
  ...
  defp wait_for_pending_deletes(ref) do
    case :ets.match(ref, {{:pending_delete, :_}}, 1) do
      :"$end_of_table" -> :ok
      _ -> wait_for_pending_deletes(ref)
    end
  end
