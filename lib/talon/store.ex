defmodule Talon.Store do
  @moduledoc false
  use GenServer
  alias Talon.Compactor
  ...
  @type data ::
          {BloomFilter.t(), Talon.db_sequence(), non_neg_integer(),
           {Talon.db_key(), Talon.db_key()}}
  ...
  @spec remove(store(), Talon.db_file()) :: :ok
  def remove(store, file) do
    GenServer.call(store, {:remove, file})
  end

  @spec get(store(), Talon.db_key()) :: [
          {(-> Talon.db_value()), (-> :ok)}
        ]
  ...
  @spec new_file(store()) :: Talon.db_file()
