defmodule Talon.SSTs.DiskTest do
  use ExUnit.Case, async: true
  alias Talon.SSTs.Disk
  ...
  test "open!/2 returns io and offset to a file", c do
    ...
    File.touch(file)
    assert %Disk{io: {:file_descriptor, :prim_file, _}, offset: 0} = Disk.open!(file)
  end

  test "open!/2 raises on error", c do
    file = Path.join(c.tmp_dir, "non-existing-file")
    assert_raise RuntimeError, fn -> Disk.open!(file) end
  end

  test "open/2 opens in read-only mode", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "bar")
    assert {:ok, %Disk{offset: 3} = disk} = Disk.open(file)
  ...
  test "open/2 opens in read-only mode at start", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "bar")
    assert {:ok, disk} = Disk.open(file, start?: true)
  ...
  test "open/2 opens in append mode", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "bar")
    assert {:ok, disk} = Disk.open(file, write?: true)
    assert {:ok, "bar"} = Disk.read(disk, 0, 3)
  ...
  test "write/2 writes to a file", c do
    file = Path.join(c.tmp_dir, "foo")
    File.touch(file)
    assert {:ok, %Disk{offset: 0} = disk} = Disk.open(file, write?: true)
  ...
  test "write/2 failes if not opened in write mode", c do
    file = Path.join(c.tmp_dir, "foo")
    File.touch(file)
    assert {:ok, %Disk{offset: 0} = disk} = Disk.open(file)
  ...
  test "read/2 reads from set offset", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "foobarbaz")
    {:ok, disk} = Disk.open(file, start?: true)
  ...
  test "advance_offset/2 adds to offset", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "foobarbaz")
    assert {:ok, %Disk{offset: 0} = disk} = Disk.open(file, start?: true)
