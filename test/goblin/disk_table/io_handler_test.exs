defmodule Goblin.DiskTable.IOHandlerTest do
  use ExUnit.Case, async: true
  alias Goblin.DiskTable.IOHandler

  @moduletag :tmp_dir

  test "open!/2 returns io and offset to a file", c do
    file = Path.join(c.tmp_dir, "foo")
    File.touch(file)
    assert %Disk{io: {:file_descriptor, :prim_file, _}, offset: 0} = IOHandler.open!(file)
  end

  test "open!/2 raises on error", c do
    file = Path.join(c.tmp_dir, "non-existing-file")
    assert_raise RuntimeError, fn -> IOHandler.open!(file) end
  end

  test "open/2 opens in read-only mode", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "bar")
    assert {:ok, %Disk{offset: 3} = disk} = IOHandler.open(file)
    assert {:ok, "bar"} = IOHandler.read(disk, 0, 3)
    assert {:error, :ebadf} == IOHandler.write(disk, "baz")
  end

  test "open/2 opens in read-only mode at start", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "bar")
    assert {:ok, disk} = IOHandler.open(file, start?: true)
    assert {:ok, "bar"} = IOHandler.read(disk, 3)
    assert {:error, :ebadf} == IOHandler.write(disk, "baz")
  end

  test "open/2 opens in append mode", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "bar")
    assert {:ok, disk} = IOHandler.open(file, write?: true)
    assert {:ok, "bar"} = IOHandler.read(disk, 0, 3)
    assert {:ok, disk} = IOHandler.write(disk, "baz")
    assert {:ok, "barbaz"} = IOHandler.read(disk, 0, 6)
  end

  test "write/2 writes to a file", c do
    file = Path.join(c.tmp_dir, "foo")
    File.touch(file)
    assert {:ok, %Disk{offset: 0} = disk} = IOHandler.open(file, write?: true)
    assert {:ok, %Disk{offset: 3}} = IOHandler.write(disk, "bar")
  end

  test "write/2 failes if not opened in write mode", c do
    file = Path.join(c.tmp_dir, "foo")
    File.touch(file)
    assert {:ok, %Disk{offset: 0} = disk} = IOHandler.open(file)
    assert {:error, :ebadf} = IOHandler.write(disk, "bar")
  end

  test "read_from_end/2 reads content from the end", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "foobarbaz")
    {:ok, disk} = IOHandler.open(file)
    assert {:ok, "baz"} == IOHandler.read_from_end(disk, 3, 3)
    assert {:ok, "bar"} == IOHandler.read_from_end(disk, 6, 3)
    assert {:ok, "foo"} == IOHandler.read_from_end(disk, 9, 3)
    assert {:error, :eof} == IOHandler.read_from_end(disk, 0, 3)
  end

  test "read/2 reads from set offset", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "foobarbaz")
    {:ok, disk} = IOHandler.open(file, start?: true)
    assert {:ok, "foo"} == IOHandler.read(disk, 3)
    assert {:ok, "foobar"} == IOHandler.read(disk, 6)
    assert {:ok, "foobarbaz"} == IOHandler.read(disk, 9)
  end

  test "read/3 reads from position", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "foobarbaz")
    {:ok, disk} = IOHandler.open(file)
    assert {:ok, "foo"} == IOHandler.read(disk, 0, 3)
    assert {:ok, "bar"} == IOHandler.read(disk, 3, 3)
    assert {:ok, "baz"} == IOHandler.read(disk, 6, 3)
    assert {:error, :eof} == IOHandler.read(disk, 9, 3)
  end

  test "advance_offset/2 adds to offset", c do
    file = Path.join(c.tmp_dir, "foo")
    File.write(file, "foobarbaz")
    assert {:ok, %Disk{offset: 0} = disk} = IOHandler.open(file, start?: true)
    assert %Disk{offset: 3} = IOHandler.advance_offset(disk, 3)
  end
end
