defmodule Goblin.Manifest.SnapshotTest do
  ...
  describe "update/2" do
    test "applying an empty list returns the snapshot unchanged" do
      snapshot = %Snapshot{sequence: 42}
      assert Snapshot.update(snapshot, []) == snapshot
    end
    ...
    test "setting wal updates the wal field and bumps wal_count" do
      snapshot = %Snapshot{}
      assert %{wal: nil, wal_count: 0} = snapshot

      assert %{wal: "wal_0.goblin", wal_count: 1} =
  ...
  describe "clear_dirt/1" do
    test "clears the dirt list" do
      snapshot =
        %Snapshot{}
        |> Snapshot.update({:disk_table_added, "foo"})
        |> Snapshot.update({:disk_table_removed, "foo"})
    ...
    test "preserves other fields when clearing dirt" do
      snapshot =
        %Snapshot{}
        |> Snapshot.update({:sequence, 42})
        |> Snapshot.update({:wal, "wal.goblin"})
        |> Snapshot.update({:disk_table_added, "foo"})
        |> Snapshot.update({:disk_table_removed, "foo"})

      snapshot = Snapshot.clear_dirt(snapshot)

      assert %{dirt: [], sequence: 42, wal: "wal.goblin"} = snapshot
    end
  end
end
