...
#
#   describe "get_total_size/1" do
#     test "returns total size of entries" do
#       level = %Level{}
...
#   end
#
#   describe "get_highest_prio_entries/1" do
#     test "returns all entries when level_key = 0" do
#       level =
#         %Level{level_key: 0}
...
#     end
#
#     test "returns entry with highest priority (minimum)" do
#       level =
#         %Level{}
#         |> Level.put_entry(%Entry{id: :foo, priority: 1})
#         |> Level.put_entry(%Entry{id: :bar, priority: 2})
...
#     test "put new entry in entries, replacing if already existing" do
#       level = %Level{}
...
#   end
#
#   describe "place_in_buffer/2" do
#     test "creates virtual entry if entries is empty" do
#       level = Level.place_in_buffer(%Level{}, {1, 0, 2})
#       assert %{entries: entries} = level
#       assert %{nil: %Entry{is_virtual: true, key_range: {1, 1}, buffer: %{1 => {0, 2}}}} = entries
#     end
#
#     test "places data in the only entry that exists" do
#       level = %Level{}
#       entry = %Entry{id: :foo}
#
#       level =
...
#         |> Level.place_in_buffer({1, 0, 2})
#
#       assert %{entries: entries} = level
#       assert %{foo: %Entry{key_range: {1, 1}, buffer: %{1 => {0, 2}}}} = entries
#
#       level = Level.place_in_buffer(level, {2, 1, 3})
#
#       assert %{entries: entries} = level
#       assert %{foo: %Entry{key_range: {1, 2}, buffer: %{1 => {0, 2}, 2 => {1, 3}}}} = entries
#     end
#
#     test "updates buffer with key in closest range" do
#       level = %Level{}
...
#              } = entries
#
#       level = Level.place_in_buffer(level, {7, 2, 8})
#       assert %{entries: entries} = level
#
#       assert %{
#                foo: %Entry{key_range: {0, 5}, buffer: %{2 => {1, 3}}},
...
#              } = entries
#
#       level = Level.place_in_buffer(level, {11, 3, 20})
#       assert %{entries: entries} = level
#
#       assert %{
#                foo: %Entry{key_range: {0, 5}, buffer: %{2 => {1, 3}}},
