defmodule Goblin.MergeTest do
  ...
  describe "stream/2" do
    test "empty streams results in empty merge" do
      assert [] == Merge.stream(fn -> [[], []] end) |> Enum.to_list()
    end
    ...
    test "merges multiple sorted streams" do
      ...
      result = Merge.stream(fn -> [s1, s2] end) |> Enum.to_list()
    ...
    test "highest seq wins when key is duplicated within stream" do
      ...
      result = Merge.stream(fn -> [s] end) |> Enum.to_list()
    ...
    test "can filter tombstones" do
      ...
      without_tombstones =
        Merge.stream(fn -> [s] end, filter_tombstones?: true) |> Enum.to_list()

      with_tombstones =
        Merge.stream(fn -> [s] end, filter_tombstones?: false) |> Enum.to_list()
    ...
    test "respects bounds" do
      ...
      assert [{1, 0, "a"}, {2, 0, "b"}] ==
               Merge.stream(fn -> [s] end, max: 2) |> Enum.to_list()

      assert [{3, 0, "c"}, {4, 0, "d"}] ==
               Merge.stream(fn -> [s] end, min: 3) |> Enum.to_list()
    end
    ...
    test ":after runs when completed" do
      ...
      Merge.stream(fn -> [s1, s2] end, after: fn -> send(me, :in_after) end)
      |> Enum.to_list()
  ...
  property "merged result is sorted by key in ascending order" do
    check all(streams <- list_of(stream_generator(), min_length: 0, max_length: 20)) do
      merged = Merge.stream(fn -> streams end)
