defmodule Goblin.DiskTables.Legacy.Legacy.EncoderTest do
  ...
  describe "encode_sst_block/2, decode_sst_header_block/1, decode_sst_block/1" do
    test "can encode and decode triple into SST block" do
      triple = {:key, 0, :val}
      assert {sst_block, 1} = Goblin.DiskTables.Legacy.Encoder.encode_sst_block(triple, false)

      assert {:ok, 1} ==
               Goblin.DiskTables.Legacy.Encoder.decode_sst_header_block(
                 :binary.part(sst_block, 0, 18)
               )

      assert {:ok, ^triple} = Goblin.DiskTables.Legacy.Encoder.decode_sst_block(sst_block)
    end
    ...
    test "can compress data in SST block" do
      ...
      assert {compressed_sst_block, 1} =
               Goblin.DiskTables.Legacy.Encoder.encode_sst_block(triple, true)
    ...
    test "can identify end of SST" do
      {bin, _, _} =
        Goblin.DiskTables.Legacy.Encoder.encode_footer_block(
          0,
          :bf,
          :key_range,
          :seq_range,
      ...
      assert {:error, :end_of_sst} ==
               Goblin.DiskTables.Legacy.Encoder.decode_sst_header_block(bin)
    end

    test "can identify invalid SST blocks" do
      assert {:error, :invalid_sst_header} ==
               Goblin.DiskTables.Legacy.Encoder.decode_sst_header_block("RANDOMBINARY")
    end
  end
    ...
    test "encodes and decodes correctly" do
      ...
      blocks =
        :binary.part(
          bin,
          0,
          byte_size(bin) - Goblin.DiskTables.Legacy.Encoder.magic_size() -
            Goblin.DiskTables.Legacy.Encoder.size_block_size() -
            Goblin.DiskTables.Legacy.Encoder.crc_block_size()
        )

      assert :ok ==
               Goblin.DiskTables.Legacy.Encoder.validate_magic_block(
                 :binary.part(bin, byte_size(bin), -Goblin.DiskTables.Legacy.Encoder.magic_size())
               )
      ...
      metadata_block =
        :binary.part(
          blocks,
          byte_size(blocks),
          -Goblin.DiskTables.Legacy.Encoder.metadata_block_size()
        )

      assert {:ok,
              {0, {bf_block_pos, bf_block_size}, {key_range_block_pos, key_range_block_size},
               {seq_range_block_pos, seq_range_block_size}, 1, false}} =
               Goblin.DiskTables.Legacy.Encoder.decode_metadata_block(metadata_block)
      ...
      assert {:ok, :a_bf} == Goblin.DiskTables.Legacy.Encoder.decode_bloom_filter_block(bf_block)

      assert {:ok, :a_key_range} ==
               Goblin.DiskTables.Legacy.Encoder.decode_key_range_block(key_range_block)

      assert {:ok, :a_seq_range} ==
               Goblin.DiskTables.Legacy.Encoder.decode_seq_range_block(seq_range_block)
    end

    test "can handle invalid blocks" do
      assert {:error, :invalid_magic} ==
               Goblin.DiskTables.Legacy.Encoder.validate_magic_block("INVALID_MAGIC")

      assert {:error, :invalid_metadata_block} ==
               Goblin.DiskTables.Legacy.Encoder.decode_metadata_block("INVALID_METADATA_BLOCK")

      assert {:error, :invalid_bloom_filter_block} ==
               Goblin.DiskTables.Legacy.Encoder.decode_bloom_filter_block(
                 :erlang.term_to_binary(:invalid_bloom_filter)
               )

      assert {:error, :invalid_key_range_block} ==
               Goblin.DiskTables.Legacy.Encoder.decode_key_range_block(
                 :erlang.term_to_binary(:invalid_key_range)
               )

      assert {:error, :invalid_seq_range_block} ==
               Goblin.DiskTables.Legacy.Encoder.decode_seq_range_block(
                 :erlang.term_to_binary(:invalid_seq_range)
               )

      assert {:error, :invalid_crc_block} ==
               Goblin.DiskTables.Legacy.Encoder.decode_crc_block("INVALID_CRC_BLOCK")

      assert {:error, :invalid_size_block} ==
               Goblin.DiskTables.Legacy.Encoder.decode_size_block("INVALID_SIZE_BLOCK")
    end
  end
end
