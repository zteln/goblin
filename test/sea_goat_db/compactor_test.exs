defmodule SeaGoatDB.CompactorTest do
  ...
  test "a completed compaction cleans up sources", c do
    ...
    assert_eventually do
      ...
      assert [
               ...
               {2, 3, :v3},
               {3, 4, :v4}
             ] == file |> SeaGoatDB.SSTs.stream!() |> Enum.to_list()
    end
  ...
  test "compaction removes tombstones in final level", c do
    ...
    assert_eventually do
      ...
      data = file |> SeaGoatDB.SSTs.stream!() |> Enum.to_list()
  ...
  test "compaction handles empty target level", c do
    ...
    assert_eventually do
      ...
      assert [%{id: file}] = Map.values(final_level.entries)
      assert [{0, 1, :v1}, {1, 2, :v2}] == file |> SeaGoatDB.SSTs.stream!() |> Enum.to_list()
    end
  end
  ...
  defp write_sst(dir, name, level_key, key_limit, data) do
