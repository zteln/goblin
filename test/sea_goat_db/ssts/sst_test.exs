defmodule SeaGoatDB.SSTs.SSTTest do
  ...
  describe "is_ss_table/1" do
    test "returns true for valid magic bytes" do
      assert SST.is_ss_table("SEAGOATDBFILE000")
    end

    test "returns false for invalid magic bytes" do
      refute SST.is_ss_table("INVALID")
      refute SST.is_ss_table("")
      refute SST.is_ss_table("SEAGOATDBFILE001")
    end
  end

  describe "block_span/1" do
    test "returns span for valid block header" do
      assert {:ok, 1} = SST.block_span(<<"SEAGOATDBBLOCK00", 1::integer-16>>)
      assert {:ok, 5} = SST.block_span(<<"SEAGOATDBBLOCK00", 5::integer-16>>)
      assert {:ok, 100} = SST.block_span(<<"SEAGOATDBBLOCK00", 100::integer-16>>)
    end

    test "returns error for separator" do
      assert {:error, :eod} = SST.block_span(<<"SEAGOATDBSEP0000", "rest">>)
    end
  ...
  describe "encode_block/3 and decode_block/1" do
    test "encodes and decodes a simple block" do
      ...
      assert <<"SEAGOATDBBLOCK00", span::integer-16, _rest::binary>> = block
      assert span == 1
  ...
  describe "encode_footer/7 and decode_metadata/1" do
    test "encodes and decodes footer metadata" do
      ...
      assert <<"SEAGOATDBSEP0000", _rest::binary>> = footer
      assert :binary.part(footer, {byte_size(footer), -16}) == "SEAGOATDBFILE000"
