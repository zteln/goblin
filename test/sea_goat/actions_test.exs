defmodule SeaGoat.ActionsTest do
  ...
  test "merge/7 merges multiple SST files into new SST files", c do
    ...
    target_level = %SeaGoat.Compactor.Level{
      level_key: 1,
      entries: %{},
      compacting_ref: nil
    }
    ...
    assert {:ok, old_files, 0, 1} =
             Actions.merge(
               ...
               0,
               target_level,
               &SeaGoat.Compactor.Level.place_in_buffer(&2, &1),
               false,
               1000,
    ...
    assert [
             ...
             {4, :k5, :v5},
             {5, :k6, :v6}
           ] == file |> SSTables.stream!() |> Enum.to_list()
  end
  ...
  test "merge/7 handles overlapping keys with sequence numbers", c do
    ...
    merged_data = SSTables.stream!(merged_file) |> Enum.to_list()
  ...
  test "merge/7 waits until wlock completes to delete merged files", c do
    ...
    reader_pid =
      spawn(fn ->
        SeaGoat.RWLocks.rlock(c.rw_locks, file)

        send(parent, {:rlocked, ref})

        receive do
          {:unlock, ^ref} -> SeaGoat.RWLocks.unlock(c.rw_locks, file)
        end
      end)
